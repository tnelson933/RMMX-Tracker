...
interface RaceResult {
  ...
  position: number | null; bibNumber: string | null; lapTimes: string[] | null;
...
export default function SeriesWidget() {
  ...
  if (loading) {
    return (
      <div style={{ fontFamily: "system-ui, sans-serif", background: "#0f1117", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#e05a1a", fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", opacity: 0.7 }}>Loading Series…</div>
      </div>
    );
  }
  ...
  if (!series) {
    return (
      <div style={{ fontFamily: "system-ui, sans-serif", background: "#0f1117", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#ef4444", fontSize: 13 }}>Series not found.</div>
      </div>
    );
  }

  return (
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", background: "#0f1117", minHeight: "100vh", color: "#f4f4f5", paddingBottom: 64 }}>

      {/* ── Header ───────────────────────────────────────────── */}
      <div style={{ background: "#161b2e", borderBottom: "1px solid #1e2740", padding: "14px 18px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            {view === "event" && (
              <button
                onClick={goBack}
                style={{ display: "inline-flex", alignItems: "center", gap: 5, color: "#e05a1a", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", background: "none", border: "none", cursor: "pointer", marginBottom: 8, padding: 0 }}
              >
                ← Series Standings
              </button>
            )}
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: "#e05a1a", marginBottom: 3 }}>
              Championship Series · {series.season}
            </div>
            <div style={{ fontSize: 19, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.03em", lineHeight: 1.2, color: "#ffffff" }}>
              {view === "event" ? selectedEventName : series.name}
            </div>
            {view === "event" && (
              <div style={{ fontSize: 11, color: "#6b7280", marginTop: 2 }}>{series.name}</div>
            )}
          </div>
          <div style={{ flexShrink: 0 }}>
            <div style={{ background: view === "standings" ? "rgba(224,90,26,0.15)" : "rgba(34,197,94,0.15)", border: `1px solid ${view === "standings" ? "rgba(224,90,26,0.4)" : "rgba(34,197,94,0.4)"}`, color: view === "standings" ? "#e05a1a" : "#22c55e", borderRadius: 4, padding: "3px 10px", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>
      ...
      {view === "standings" && (
        <>
          {classes.length === 0 ? (
            ...
            <>
              {/* Class tabs */}
              <div style={{ padding: "0 18px", borderBottom: "1px solid #1e2740", display: "flex", gap: 2, overflowX: "auto" }}>
                {classes.map(cls => (
                  <button key={cls} onClick={() => setActiveClass(cls)}
                    style={{ padding: "10px 14px", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", border: "none", borderBottom: cls === displayClass ? "2px solid #e05a1a" : "2px solid transparent", background: "none", color: cls === displayClass ? "#e05a1a" : "#6b7280", cursor: "pointer", whiteSpace: "nowrap" }}>
                    {cls}
                  </button>
                ))}
              </div>
              ...
              {series.events.length > 0 && (
                <div style={{ padding: "10px 18px", borderBottom: "1px solid #1e2740", display: "flex", gap: 6, overflowX: "auto", alignItems: "center" }}>
                  <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#374151", whiteSpace: "nowrap" }}>Races:</span>
                  {series.events.map((ev, i) => (
                    <button key={ev.id} onClick={() => goToEvent(ev.id, ev.name, displayClass)}
                      style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "#1a2035", border: "1px solid #2a3a5c", borderRadius: 5, color: "#94a3b8", fontSize: 11, fontWeight: 600, padding: "4px 10px", cursor: "pointer", whiteSpace: "nowrap" }}>
                      <span style={{ color: "#e05a1a", fontSize: 10, fontWeight: 800 }}>{i + 1}</span>
                      {ev.name}
                      {ev.status === "race_day" && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#ef4444", display: "inline-block" }} />}
                    </button>
                  ))}
                </div>
              )}
              ...
              {classStandings.length === 0 ? (
                ...
                <div style={{ overflowX: "auto" }}>
                  {/* Column headers */}
                  <div style={{ display: "grid", gridTemplateColumns: `38px 1fr${eventColumns.map(() => " 70px").join("")} 52px 64px`, padding: "8px 18px", borderBottom: "1px solid #1e2740", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#374151", minWidth: 320, gap: 0 }}>
                    <span>Pos</span>
                    <span>Rider</span>
                    {eventColumns.map((ev, i) => (
                      <button key={ev.eventId} onClick={() => goToEvent(ev.eventId, ev.eventName, displayClass)}
                        style={{ textAlign: "center", background: "none", border: "none", color: "#4b5563", cursor: "pointer", padding: "0 2px", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", textDecoration: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
                        <span style={{ color: "#e05a1a", fontSize: 8, fontWeight: 800 }}>R{i + 1}</span>
                  ...
                  {classStandings.map(row => (
                    <div key={row.riderId} style={{ display: "grid", gridTemplateColumns: `38px 1fr${eventColumns.map(() => " 70px").join("")} 52px 64px`, padding: "9px 18px", borderBottom: "1px solid #111827", alignItems: "center", minWidth: 320, background: row.position === 1 ? "rgba(224,90,26,0.04)" : "transparent" }}>
                      {/* Pos */}
                      <div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 3, minWidth: 0, paddingRight: 6 }}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
                          {row.amaNumber && (
                            <span style={{ fontSize: 10, color: "#6b7280" }}>AMA# {row.amaNumber}</span>
                          )}
                      ...
                      {eventColumns.map(ev => {
                        ...
                        return (
                          <div key={ev.eventId} style={{ textAlign: "center" }}>
                            {evData?.attended ? (
                              <div>
                                <div style={{ fontSize: 12, fontWeight: 800, color: "#f4f4f5" }}>{evData.eventScore}</div>
                                <div style={{ fontSize: 9, color: "#4b5563" }}>{evData.motos?.join("·")}</div>
                              </div>
                            ) : (
                      ...
                      {/* Events entered */}
                      <div style={{ textAlign: "center", fontSize: 12, color: "#6b7280" }}>{row.eventsEntered}</div>
                      {/* Total */}
                      <div style={{ textAlign: "right", fontSize: 15, fontWeight: 900, color: "#e05a1a" }}>{row.totalScore}</div>
                    </div>
                  ))}
                </div>
              )}
      ...
      {view === "event" && (
        <>
          {/* Search */}
          <div style={{ padding: "10px 18px", borderBottom: "1px solid #1e2740" }}>
            ...
            <div style={{ padding: "0 18px", borderBottom: "1px solid #1e2740", display: "flex", gap: 2, overflowX: "auto" }}>
              {eventClasses.map(cls => (
                <button key={cls} onClick={() => { setActiveEventClass(cls); setExpanded(null); }}
                  style={{ padding: "10px 14px", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", border: "none", borderBottom: cls === displayEventClass ? "2px solid #e05a1a" : "2px solid transparent", background: "none", color: cls === displayEventClass ? "#e05a1a" : "#6b7280", cursor: "pointer", whiteSpace: "nowrap" }}>
                  {cls}
                </button>
              ))}
            </div>
          ...
          {eventLoading ? (
            <div style={{ padding: "40px", textAlign: "center", color: "#4b5563", fontSize: 13 }}>Loading results…</div>
            ...
            <>
              {/* Column header */}
              <div style={{ display: "grid", gridTemplateColumns: "40px 1fr 50px 78px 78px", padding: "8px 18px", borderBottom: "1px solid #1e2740", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#374151" }}>
                ...
                return (
                  <div key={rider.id} style={{ borderBottom: "1px solid #111827" }}>
                    <div role="button" tabIndex={0}
                      ...
                      style={{ display: "grid", gridTemplateColumns: "40px 1fr 50px 78px 78px", padding: "10px 18px", cursor: "pointer", background: isExpanded ? "#161b2e" : "transparent" }}>
                      {/* Pos */}
                      ...
                      <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 3, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
                          {rider.bibNumber && <span style={{ fontSize: 10, color: "#6b7280" }}>#{rider.bibNumber}</span>}
                          {rInfo?.amaNumber && <span style={{ fontSize: 10, color: "#6b7280" }}>AMA# {rInfo.amaNumber}</span>}
                          ...
                          {rider.dnf && <span style={{ color: "#ef4444", fontSize: 10 }}>DNF</span>}
                          {rider.dns && <span style={{ color: "#6b7280", fontSize: 10 }}>DNS</span>}
                        </div>
                      </div>
                      {/* Laps */}
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#94a3b8" }}>{lapsToShow.length || "—"}</div>
                      {/* Best */}
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", fontSize: 12, color: "#34d399", fontVariantNumeric: "tabular-nums" }}>{fmtMs(bl)}</div>
                      {/* Total */}
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", fontSize: 12, color: "#94a3b8", fontVariantNumeric: "tabular-nums" }}>{totalMs && totalMs > 0 ? fmtMs(totalMs) : "—"}</div>
                    </div>

                    {/* Expanded lap history */}
                    {isExpanded && (
                      <div style={{ background: "#0d1120", padding: "12px 18px 16px", borderTop: "1px solid #1a2035" }}>
                          ...
                          return (
                            <div key={moto.id} style={{ marginBottom: 14 }}>
                              <div style={{ fontSize: 11, fontWeight: 700, color: "#dc2626", marginBottom: 8, display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                {laps.length > 0 && <span style={{ marginLeft: "auto", color: "#4b5563", fontSize: 10 }}>Best: <span style={{ color: "#34d399" }}>{fmtMs(mbl)}</span></span>}
                              </div>
                              {laps.length > 0 ? (
