...
if (!CLOUD_URL || !CLUB_ID || !EMAIL || !PASSWORD) {
  ...
  Usage:
    ...
    pnpm --filter @workspace/scripts run sync-to-cloud

  Or positional args:
    pnpm --filter @workspace/scripts run sync-to-cloud \\
      ./race_data.db https://your-app.replit.app 1 jake@club.com secret
`);
  process.exit(1);
}
...
try {
  ...
  console.error(
    ...
    "  Re-run import-data to create it, then retry sync.",
  );
  process.exit(1);
}
...
if (watermarks["checkins"])        console.log(`\n  Watermarks (max id at import):`);
if (watermarks["checkins"])        console.log(`    checkins:         ${watermarks["checkins"]}`);
if (watermarks["rfid_assignments"]) console.log(`    rfid_assignments: ${watermarks["rfid_assignments"]}`);
if (watermarks["registrations"])   console.log(`    registrations:    ${watermarks["registrations"]}`);
if (watermarks["riders"])          console.log(`    riders:           ${watermarks["riders"]}`);
...
// Extract session cookie — set-cookie header is "name=value; Path=...; ..."
...
console.log(`  Logged in. Syncing to cloud…\n`);
...
console.log(`  ✓  Sync Results (${result.syncedAt}):`);
